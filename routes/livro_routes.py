    ...
    try:
        ...
        if google_data and 'items' in google_data and len(google_data['items']) > 0:
            ...
            book_data_preview = {
                ...
                'id_prateleira': None, # Novo campo
                'localizacao_fisica': None, # Removido do DB, mas mantido para compatibilidade temporária se necessário
                'notas_pessoais': None,
            ...
            return jsonify({
                ...
                "livro": {'isbn': isbn, 'titulo': 'Título Desconhecido', 'autores': 'Autor Desconhecido', 'genero': None, 'editora': None, 'ano_publicacao': None, 'numero_paginas': None, 'capa_url': None, 'idioma': 'pt', 'id_prateleira': None, 'localizacao_fisica': None, 'notas_pessoais': None, 'data_inicio_leitura': None, 'data_fim_leitura': None, 'id_usuario': None}
            })
...
def adicionar_livro(current_user_id): # Recebe o ID do usuário logado
    ...
    isbn_from_data = data.get('isbn') 

    conn = get_db_connection()
    if conn:
        cursor = conn.cursor()
        try:
            # Verifica se a prateleira existe e pertence ao usuário, se id_prateleira for fornecido
            id_prateleira = data.get('id_prateleira')
            if id_prateleira is not None:
                cursor.execute("SELECT id FROM prateleiras WHERE id = %s AND id_usuario = %s", (id_prateleira, current_user_id))
                prateleira_existe = cursor.fetchone()
                if not prateleira_existe:
                    return jsonify({"status": "erro", "mensagem": "Prateleira não encontrada ou você não tem permissão para usá-la."}), 400

            sql = """
            INSERT INTO livros (
                isbn, titulo, autores, genero, editora, ano_publicacao,
                numero_paginas, capa_url, id_prateleira, notas_pessoais,
            ...
            values = (
                ...
                data.get('titulo'),
                data.get('autores'),
                data.get('genero', None),
                data.get('editora', None),
                data.get('ano_publicacao', None),
                data.get('numero_paginas', None),
                data.get('capa_url', None),
                id_prateleira, # Usa o id_prateleira
                data.get('notas_pessoais', None),
                data.get('idioma', None),
                data.get('data_inicio_leitura', None),
                data.get('data_fim_leitura', None),
                current_user_id
            )
            ...
            return jsonify({
                "status": "sucesso",
                "mensagem": f"Livro com ISBN {isbn_from_data} adicionado com sucesso.",
            ...
            if "Duplicate entry" in str(err) and "isbn" in str(err).lower():
                 return jsonify({"status": "erro", "mensagem": f"Livro com ISBN {isbn_from_data} já existe no catálogo."}), 409
...
def get_all_livros(current_user_id): # Recebe o ID do usuário logado
    ...
    if conn:
        ...
        try:
            ...
            # Modificado para fazer JOIN com prateleiras e estantes
            sql = """
            SELECT DISTINCT l.*, p.nome AS nome_prateleira, e.nome AS nome_estante
            FROM livros l
            LEFT JOIN prateleiras p ON l.id_prateleira = p.id
            LEFT JOIN estantes e ON p.id_estante = e.id
            """
            values = []
            ...
            valid_order_fields = ['titulo', 'autores', 'ano_publicacao', 'data_cadastro', 'id', 'nome_prateleira', 'nome_estante']
...
def get_livro_by_id(current_user_id, livro_id): # Recebe o ID do usuário
    ...
    if conn:
        ...
        try:
            # Modificado para fazer JOIN com prateleiras e estantes
            sql = """
            SELECT l.*, p.nome AS nome_prateleira, e.nome AS nome_estante
            FROM livros l
            LEFT JOIN prateleiras p ON l.id_prateleira = p.id
            LEFT JOIN estantes e ON p.id_estante = e.id
            WHERE l.id = %s AND l.id_usuario = %s
            """
...
def update_livro(current_user_id, livro_id): # Recebe o ID do usuário
    ...
    if conn:
        ...
        try:
            ...
            updatable_fields = [
                'isbn', 'titulo', 'autores', 'genero', 'editora', 'ano_publicacao',
                'numero_paginas', 'capa_url', 'id_prateleira', 'notas_pessoais', # id_prateleira agora
